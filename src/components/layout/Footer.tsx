import Link from 'next/link';
import { SITE_CONFIG, NAV_LINKS } from '@/lib/constants';
import { getWhatsAppUrl } from '@/lib/utils';

export function Footer() {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="border-t border-neutral-200 bg-white">
      <div className="container-custom py-12">
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
          {/* Brand */}
          <div className="lg:col-span-2">
            <Link href="/" className="text-xl font-semibold text-neutral-900">
              {SITE_CONFIG.shortName}
            </Link>
            <p className="mt-3 max-w-md text-neutral-600">{SITE_CONFIG.tagline}</p>
            <p className="mt-4 text-sm text-neutral-500">
              Asesoría personalizada en cuidado de la piel con productos de marca internacional.
            </p>
          </div>

          {/* Links */}
          <div>
            <h3 className="font-semibold text-neutral-900">Navegación</h3>
            <ul className="mt-4 space-y-2">
              {NAV_LINKS.slice(0, 5).map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-neutral-600 transition-colors hover:text-primary-600"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h3 className="font-semibold text-neutral-900">Contacto</h3>
            <ul className="mt-4 space-y-2">
              <li>
                <a
                  href={getWhatsAppUrl('contacto')}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 text-neutral-600 transition-colors hover:text-green-600"
                >
                  <WhatsAppIcon />
                  WhatsApp
                </a>
              </li>
              <li>
                <Link
                  href="/contacto"
                  className="text-neutral-600 transition-colors hover:text-primary-600"
                >
                  Página de contacto
                </Link>
              </li>
              <li>
                <Link
                  href="/legal"
                  className="text-neutral-600 transition-colors hover:text-primary-600"
                >
                  Información legal
                </Link>
              </li>
            </ul>
          </div>
        </div>

        {/* Disclaimer */}
        <div className="mt-8 rounded-lg bg-neutral-50 p-4">
          <p className="text-xs text-neutral-500">
            <strong>Aviso importante:</strong> Este sitio ofrece asesoría cosmética orientativa y no
            reemplaza la consulta con un dermatólogo u otro profesional de la salud. Los resultados
            pueden variar según cada persona. Este sitio no es el sitio oficial de Nu Skin ni
            representa a la marca de forma oficial. Los productos se adquieren a través del portal
            autorizado de la marca.
          </p>
        </div>

        {/* Copyright */}
        <div className="mt-8 flex flex-col items-center justify-between gap-4 border-t border-neutral-200 pt-8 md:flex-row">
          <p className="text-sm text-neutral-500">
            &copy; {currentYear} {SITE_CONFIG.shortName}. Todos los derechos reservados.
          </p>
          <div className="flex gap-4">
            <Link href="/legal" className="text-sm text-neutral-500 hover:text-neutral-700">
              Términos y condiciones
            </Link>
            <Link href="/legal" className="text-sm text-neutral-500 hover:text-neutral-700">
              Política de privacidad
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
}

function WhatsAppIcon() {
  return (
    <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
    </svg>
  );
}
